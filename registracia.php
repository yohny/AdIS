...
<form name="reg_form" action="actions/registruj.php" method="POST">
  ...
  <table cellspacing="5" style="text-align:left">
    <tr title="Login slúžiaci na prihlásenie do systému. Musí byť jedinečný.">
      ...
      <td><input id="login" type="text" name="user[login]" maxlength="10" onKeyUp="overLogin(this.value);" <?php if(isset($user)) echo 'value="'.$user['login'].'"'; ?>><br>
      <span id="loginStatus"></span></td>
    </tr>
    <tr title="Heslo pre prístup do systému.">
      <td><label for="pass">Heslo:</label></td>
      <td><input id="pass" type="password" name="user[heslo]" maxlength="10" <?php if(isset($user)) echo 'value="'.$user['heslo'].'"'; ?>></td>
    </tr>
    <tr title="Potvrdenie hesla do systému.">
      <td><label for="pass2">Heslo znova:</label></td>
      <td><input id="pass2" type="password" name="user[heslo2]" maxlength="10" <?php if(isset($user)) echo 'value="'.$user['heslo2'].'"'; ?>></td>
    </tr>
    <tr title="Vaša webová adresa vrátane protokolu (http://) napr.: 'http://www.vasafirma.sk'.">
      <td><label for="web">WWW adresa:</label></td>
      <td><input id="web" type="text" name="user[web]" maxlength="100" <?php if(isset($user)) echo 'value="'.$user['web'].'"'; ?>></td>
    </tr>
    <tr title="Zvoľte 'Inzerent' ak chcete pridať svoje reklamy, ktoré bolú zobrazované inými používateľmi na ich stránkach.">
      ...
      <td><input id="inzer" type="radio" name="user[skupina]" value="inzer" checked="checked"></td>
    </tr>
    <tr title="Zvoľte 'Zobrazovateľ' ak chcete zobrazovať reklamy íných používateľov na svojich stránkach.">
      ...
      <td><input id="zobra" type="radio" name="user[skupina]" value="zobra" <?php if(isset($user) && $user['skupina']=='zobra') echo 'checked="checkeded"'; ?>></td>
    </tr>
    <tr>
