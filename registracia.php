...
<form name="reg_form" action="actions/registruj.php" method="POST" autocomplete="off">
  ...
  <table cellspacing="5" style="text-align:left">
    ...
    <tr title="Heslo pre prístup do systému.">
      ...
      <td><input type="password" name="user[heslo]" id="user_heslo" maxlength="10" <?php if(isset($user)) echo 'value="'.$user['heslo'].'"'; ?>></td>
    </tr>
    <tr title="Potvrdenie hesla do systému.">
      <td><label for="user_heslo2">Heslo znova:</label></td>
      <td><input type="password" name="user[heslo2]" id="user_heslo2" maxlength="10" <?php if(isset($user)) echo 'value="'.$user['heslo2'].'"'; ?>></td>
    </tr>
    <tr title="Vaša webová adresa vrátane protokolu (http://) napr.: 'http://www.vasafirma.sk'.">
      <td><label for="user_web">WWW adresa:</label></td>
      <td><input type="text" name="user[web]" id="user_web" maxlength="100" <?php if(isset($user)) echo 'value="'.$user['web'].'"'; ?>></td>
    </tr>
    ...
    <tr title="Zvoľte 'Zobrazovateľ' ak chcete zobrazovať reklamy íných používateľov na svojich stránkach.">
      ...
      <td><input type="radio" name="user[skupina]" id="user_zobra" value="zobra" <?php if(isset($user) && $user['skupina']=='zobra') echo 'checked="checkeded"'; ?>></td>
    </tr>
    <tr>
